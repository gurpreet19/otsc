 ...
 */
@Component
public class TMFProductOfferingPriceRouter extends RouteBuilder {
	
	
	private final Logger log = LoggerFactory.getLogger(TMFProductOfferingPriceRouter.class);
	  
    @Override
    public void configure() throws Exception {
    	
    	from("direct:offerToTMFProductOfferingPriceRoute")
    	.bean(TMFProductOfferingPriceModel.class) 
    	...
    	.process(new Processor() {
            public void process(Exchange exchange) throws Exception { 
              	...
              	log.debug("Calling TMF Product Offering Price Publish API " + exchange.getIn().getBody());
              }
          })    	 
    	.to("http://51.144.48.184:8082/api/v1/productofferingprice")
    	 ...
    	 .process(new Processor() {
			...
			public void process(Exchange exchange) throws Exception {
	           ...
	           if(code == 200 || code == 201){
	            	log.debug("Product Offering Price: " + exchange.getIn() + " Published Succesfully");
	            }
	            else{
	            	log.debug("Failed To Publish Product Offering Price: " + exchange.getIn().getBody());
	            }
			}
