 ...
 */
@Component
public class TMFServiceSpecificationRouter extends RouteBuilder {
	
	
	private final Logger log = LoggerFactory.getLogger(TMFServiceSpecificationRouter.class);
	  
    @Override
    public void configure() throws Exception {
    	
    	from("direct:serviceToTMFServiceSpecificationRoute")
    	.bean(TMFServiceSpecificationModel.class) 
    	...
    	.process(new Processor() {
            public void process(Exchange exchange) throws Exception { 
              	...
              	log.debug("Calling TMF Service Specification Publish API " + exchange.getIn().getBody());
              }
          })    	 
    	.to("http://51.144.48.184:8082/api/v1/servicespecification")
    	 ...
    	 .process(new Processor() {
			...
			public void process(Exchange exchange) throws Exception {
	           ...
	           if(code == 200 || code == 201){
	            	log.debug("Service Specification: " + exchange.getIn() + " Published Succesfully");
	            }
	            else{
	            	log.debug("Failed To Publish Service Specification: " + exchange.getIn().getBody());
	            }
			}
