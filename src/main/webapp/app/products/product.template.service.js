(function() {
    ...
    ProductTemplate.$inject = ['$http'];

    function ProductTemplate ($http) {
      var factory = {};

      factory.get_template_schema_form = function(template_file_name) {
        console.log("inside get_template_schema_form ProductTemplate service");
        ...
        return $http.get("/content/otsc-templates/products/" + template_file_name).then(
          function(data, status, headers, config) {
            ...
            var product_parameters = template.product_parameters;
            ...
            for(var i = 0; i < product_parameters.length; i++) {
              var required = [];
              var defaultValue;
              var field_group_name = product_parameters[i].name;
              // list of fields in the field group, taken from YAML template
              var fields = product_parameters[i].params;
              // field_group holds all the field definitions in a group of fields in
              // "angular json schema" format
              var field_group = {};
              ...
              // add the field group to the schema
              template_schema[field_group_name] = {
                "title":       product_parameters[i].desc,
                "type":       "object",
                "properties": field_group
              };
              template_schema[field_group_name].required = required;
